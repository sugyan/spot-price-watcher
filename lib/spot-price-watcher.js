...
class SpotPriceWatcher extends EventEmitter {
    ...
    execute() {
        ...
        if (this.opts.auto_refresh) {
            setInterval(() => this.fetchPrices(), this.opts.auto_refresh * 1000 * 60);
        }
    }
    fetchPrices() {
        return this.collector.fetchPrices(moment().subtract(1, 'hour').toDate()).then((results) => {
    ...
    render() {
        ...
        const series = Object.keys(this.data).sort().map((zone, i) => {
            return {
                ...
                style: {
                    line: i < this.colors.length ? this.colors[i] : [0xff, 0xff, 0xff]
                }
            };
        });
